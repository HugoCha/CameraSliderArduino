...
const char phi_prompt_lcd_ch6[] PROGMEM ={64,B01110,B10001,B00100,B01010,64,B00100,64,0};///< Custom LCD character: Wifi
...
//  lcd->noBlink();
  for (byte i=0;i<rows;i++)
  {
    ...
    if ((para->low.i+inc>=strlen_P(para->ptr.msg_P))|| ch=='\n')
    {
      //ch=0;
      inc++;
      ch = (para->low.i+inc>=strlen_P(para->ptr.msg_P))?0:pgm_read_byte_near(para->ptr.msg_P+para->low.i+inc);
    }
    lcd->setCursor(para->col,para->row+i);
    for (byte j=0;j<columns;j++)
    {
 ...
 */
int ok_dialog(char msg[], const uint8_t row)
{
  phi_prompt_struct yn_list;
...
// Use the yn_list struct to display the message as a long message to enable multiple line question.
  ...
  yn_list.step.c_arr[1]=lcd_w; // column
  yn_list.col=0; // Display the text area starting at column 0
  yn_list.row=row; // Display the text area starting at row 0
  yn_list.option=0; // Option 0, display classic message, option 1, display message with scroll bar on right.
